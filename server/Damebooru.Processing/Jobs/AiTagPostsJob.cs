...
{
    private sealed record AiTagCandidatePath(int PostId, int LibraryId, string RelativePath);
        ...
        CancellationToken cancellationToken)
    {
        ...
        return await FilterAutoTagExcludedPostIdsAsync(db, candidatePostIds, cancellationToken);
    }

    private static async Task<List<int>> FilterAutoTagExcludedPostIdsAsync(
        DamebooruDbContext db,
        ...
        CancellationToken cancellationToken)
    {
        ...
        for (var i = 0; i < candidatePostIds.Count; i += batchSize)
        {
            ...
            var candidatePaths = await db.PostFiles
                ...
                .Select(pf => new AiTagCandidatePath(pf.PostId, pf.LibraryId, pf.RelativePath))
                .ToListAsync(cancellationToken);
