...
{
    /// <inheritdoc />
    public partial class CountDistinctPostsPerTag : Migration
    {
        /// <inheritdoc />
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            ...
            migrationBuilder.Sql(
                ...
                WHEN (SELECT COUNT(*) FROM PostTags WHERE TagId = NEW.TagId AND PostId = NEW.PostId) = 1
                BEGIN
                    UPDATE Tags
            ...
            migrationBuilder.Sql(
                ...
                WHEN (SELECT COUNT(*) FROM PostTags WHERE TagId = OLD.TagId AND PostId = OLD.PostId) = 0
                BEGIN
                    UPDATE Tags
            ...
            migrationBuilder.Sql(
                ...
                AFTER UPDATE OF PostId, TagId ON PostTags
                WHEN OLD.PostId <> NEW.PostId OR OLD.TagId <> NEW.TagId
                BEGIN
                    UPDATE Tags
                    SET PostCount = CASE WHEN PostCount > 0 THEN PostCount - 1 ELSE 0 END
                    WHERE Id = OLD.TagId
                        AND (SELECT COUNT(*) FROM PostTags WHERE TagId = OLD.TagId AND PostId = OLD.PostId) = 0;

                    UPDATE Tags
                    SET PostCount = PostCount + 1
                    WHERE Id = NEW.TagId
                        AND (SELECT COUNT(*) FROM PostTags WHERE TagId = NEW.TagId AND PostId = NEW.PostId) = 1;
                END;
                """);
