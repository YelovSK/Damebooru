...
export class BulkTaggingComponent {
    ...
    async startTagging() {
        ...
        if (postsToTag.length === 0) {
            this.toast.show('All selected posts are already queued or processed', 'info');
            return;
        }

        this.isTagging.set(true);
        this.toast.show(`Queuing ${postsToTag.length} posts for tagging...`, 'info');
                ...
                this.autoTagging
                    ...
                    .subscribe({
                        next: (entry) => {
                            ...
                            const state: TaggingState = entry.status === 'completed'
                                ? (entry.results?.some(r => r.categorizedTags.length > 0) ? 'success' : 'no-results')
                                : entry.status === 'failed' ? 'error' : entry.status;
    ...
    applyTagsToPost(postId: number) {
        ...
        this.bakabooru
            ...
            .subscribe({
                next: () => {
                    this.toast.show(`Applied ${allTags.size} tags to post #${postId}`, 'success');
    ...
    private updatePostStatus(postId: number, state: TaggingState, result?: TaggingEntry) {
        this.postStatuses.update((map) => {
            const newMap = new Map(map);
            const existing = newMap.get(postId);
            if (existing) {
                newMap.set(postId, { ...existing, state, result: result ?? existing.result });
            }
            return newMap;
        });
    }
