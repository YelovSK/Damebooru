...
export class BakabooruService {
  ...
  getPosts(
    ...
    return this.http.get<BakabooruPostListDto>(`${this.baseUrl}/posts`, {
      params,
    });
  }
  ...
  getTags(
    ...
    offset = 0,
    limit = 100,
  ): Observable<PagedSearchResult<BakabooruTagDto>> {
    const page = Math.floor(offset / limit) + 1;
    const params = new HttpParams()
      .set("query", query)
      .set("page", page.toString())
    ...
    return this.http
      .get<
        BakabooruPagedResponse<BakabooruTagDto>
  ...
  getPostsAround(id: number, query = ""): Observable<BakabooruPostsAroundDto> {
    ...
    return this.http.get<BakabooruPostsAroundDto>(
  ...
  getManagedTags(
    ...
    return this.http
      .get<
        BakabooruPagedResponse<ManagedTag>
      >(`${this.baseUrl}/tags`, { params })
      .pipe(
        map((response) => ({
          query,
